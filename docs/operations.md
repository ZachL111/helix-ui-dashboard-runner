...
`helix-ui-dashboard-runner` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for dashboard scenarios with node-edge fixtures, cycle and reachability reports, and no production deployment claims.
...
The score is deterministic. The current threshold is 164. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 2.
